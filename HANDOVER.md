...
| 评分规则配置表 | 生产评分价格维度的唯一来源，维护各语言对 AIPE/人工翻译预期价和上限价 | 语言对、AIPE 预期/上限、翻译预期/上限、规则版本、启用状态 | 读取命中的规则来源和版本，不写业务字段 | 规则变化、语种价格变化、评分策略调整时维护 |
...
- 评分规则配置表是生产评分必需表；规则变更会影响评分和评级，必须记录维护人、变更原因和生效时间。
